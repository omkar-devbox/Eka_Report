"""
Trim Loss Report — FastAPI Router
===================================
Generates an Excel report (based on Trim_loss_Report.xlsx template) populated with:

  Summary block (A7:M12)
  ├── One row per trim station (CT-10 … CT-60, stations 10-60 mapped to CT_S1-CT_S6)
  ...
  Station Call History (row 15 onwards)
  └── All raw records from CT_S1–CT_S6 for the requested time window, ordered by DT ASC.
...
from app.schemas.trim_loss_report import TrimLossReportRequest, OpenTrimFileRequest
from app.constant.trim_loss_queries import TrimLossReportSummary, TrimLossReportHistory
...
TEMPLATE_PATH = BASE_DIR / "Trim_loss_Report.xlsx"
...
# ---------------------------------------------------------------------------
#
# Row 7  -> Station CT-10 (CT_S1)
# Row 8  -> Station CT-20 (CT_S2)
# Row 9  -> Station CT-30 (CT_S3)
# Row 10 -> Station CT-40 (CT_S4)
# Row 11 -> Station CT-50 (CT_S5)
# Row 12 -> Station CT-60 (CT_S6)
#
...
# ---------------------------------------------------------------------------
@router.post("/trim-loss-report")
def generate_trim_loss_report(
    payload: TrimLossReportRequest,
    conn: pyodbc.Connection = Depends(get_db_connection),
):
    """
    Generates the Trim Loss Report Excel file from the Trim_loss_Report.xlsx template.
    ...
    - Station Call History (rows 15+): all raw CT_Sx records in the time window, with thin borders
    ...
    try:
        ...
        output_path = downloads_dir / f"TrimLossReport_{now_str}.xlsx"
        ...
        cursor.execute(TrimLossReportSummary(payload.StartTime, payload.EndTime))
        ...
        cursor.execute(TrimLossReportHistory(payload.StartTime, payload.EndTime))
        ...
        for idx, h_row in enumerate(history_rows):
            ...
            # Set thin borders for columns 1 to 7 (A to G) to enclose the data row
            for c in range(1, 8):
        ...
        print(
            f"[TrimLossReport] Generated: {output_path} | "
...
# ---------------------------------------------------------------------------
@router.post("/trim-loss-report/open-file")
def open_trim_file(payload: OpenTrimFileRequest):
...
# ---------------------------------------------------------------------------
@router.post("/trim-loss-report/open-folder")
def open_trim_folder(payload: OpenTrimFileRequest):
