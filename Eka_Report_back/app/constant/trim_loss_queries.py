"""
SQL Query functions for the Trim Loss Report.
...
Tables used: dbo.CT_S1 through dbo.CT_S6
Expected CT_Sx schema columns:
...
def TrimLossReportSummary(start_time: str, end_time: str) -> str:
    """
    ...
    Station mapping:
        CT_S1 -> Station 10  (CT-10)
        CT_S2 -> Station 20  (CT-20)
        CT_S3 -> Station 30  (CT-30)
        CT_S4 -> Station 40  (CT-40)
        CT_S5 -> Station 50  (CT-50)
        CT_S6 -> Station 60  (CT-60)

    Result columns per row (one row per station):
        StationLabel    VARCHAR   e.g. 'CT-10'
...
WITH AllStations AS
(
    SELECT 'CT-10' AS StationLabel, Typeofcall, LossTime FROM dbo.CT_S1
    WHERE DT BETWEEN @StartTime AND @EndTime

    UNION ALL
    SELECT 'CT-20', Tpeofcall AS Typeofcall, LosstTime AS LossTime FROM dbo.CT_S2
    WHERE DT BETWEEN @StartTime AND @EndTime

    UNION ALL
    SELECT 'CT-30', Typeofcall, LossTime FROM dbo.CT_S3
    WHERE DT BETWEEN @StartTime AND @EndTime

    UNION ALL
    SELECT 'CT-40', Typeofcall, LossTime FROM dbo.CT_S4
    WHERE DT BETWEEN @StartTime AND @EndTime

    UNION ALL
    SELECT 'CT-50', Typeofcall, LossTime FROM dbo.CT_S5
    WHERE DT BETWEEN @StartTime AND @EndTime

    UNION ALL
    SELECT 'CT-60', Typeofcall, LossTime FROM dbo.CT_S6
    WHERE DT BETWEEN @StartTime AND @EndTime
),

StationList AS
(
    SELECT 'CT-10' AS StationLabel, 1 AS SortOrder
    UNION ALL SELECT 'CT-20', 2
    UNION ALL SELECT 'CT-30', 3
    UNION ALL SELECT 'CT-40', 4
    UNION ALL SELECT 'CT-50', 5
    UNION ALL SELECT 'CT-60', 6
)
...
def TrimLossReportHistory(start_time: str, end_time: str) -> str:
    """
    Returns all raw call records from CT_S1 through CT_S6 within [start_time, end_time],
...
SELECT
    DT, 'CT-20', 20, Tpeofcall AS Typeofcall,
    CASE
        WHEN Tpeofcall = 1 THEN 'Process Call'
        WHEN Tpeofcall = 2 THEN 'Material Call'
        WHEN Tpeofcall = 3 THEN 'Quality Call'
        WHEN Tpeofcall = 4 THEN 'Maintenance Call'
        ELSE 'Other'
    END,
    ISNULL(LosstTime, 0),
    DATEADD(SECOND, ISNULL(LosstTime, 0), DT),
