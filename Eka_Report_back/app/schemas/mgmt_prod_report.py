...
class ProdReportType2Request(BaseModel):
    """
    Request schema for the Production Report Type-2 endpoint
    (populates ProductionReport_R3.xlsx).
    Accepts the same date/shift parameters as MgmtProdReportRequest.
    """
    ReportDate: str = Field(..., description="Report Date in YYYY-MM-DD format", examples=["2026-06-09"])
    StartDate: str = Field(..., description="Start of Financial Year in YYYY-MM-DD format", examples=["2026-04-01"])
