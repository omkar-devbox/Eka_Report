...
export default function ChassisLossReport() {
  ...
  const handleDownloadReport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient<GenerateReportResponse>("/api/chassis-loss-report", {
        body: payload
      });
      ...
      setToast({
        type: "success",
        message: "Chassis Loss Report generated successfully!"
      });
    } catch (err: any) {
      console.error("Download error:", err);
      setToast({
        type: "error",
        message: err.message || "Failed to download the chassis loss report. Check your database connection."
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleOpenFile = async () => {
    ...
    try {
      await apiClient("/api/chassis-loss-report/open-file", {
  ...
  const handleOpenFolder = async () => {
    ...
    try {
      await apiClient("/api/chassis-loss-report/open-folder", {
        body: { filepath: lastGeneratedFile.filepath }
      });
      setToast({
        type: "success",
    ...
    <div className="relative font-sans text-text-primary select-none max-w-7xl mx-auto py-2 flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl md:text-3xl font-extrabold font-display text-slate-900 tracking-tight">
          Chassis Loss Report Hub
        </h2>
        <p className="text-sm text-text-secondary max-w-3xl">
          Generate consolidated Microsoft Excel spreadsheets containing chassis station loss aggregates, call frequencies, and detailed station call history records.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 relative overflow-hidden group hover:border-slate-300 transition-all duration-300 shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-900 mt-2">CH-10 to CH-60</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 md:p-8 flex flex-col justify-between shadow-md relative">
          <form id="chassis-report-form" onSubmit={handleDownloadReport} className="flex flex-col gap-6 w-full">
            <div>
            ...
            <div className="mt-4 border-t border-slate-100 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <button
                id="generate-chassis-report-btn"
                type="submit"
                disabled={isLoading}
